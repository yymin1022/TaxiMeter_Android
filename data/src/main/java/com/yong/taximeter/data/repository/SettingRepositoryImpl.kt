...
class SettingRepositoryImpl @Inject constructor(
    ...
    override fun getCurrentRegion(): RegionSetting {
        val regionKey = preferenceDataSource.getString(PreferenceDefs.PREF_KEY_SETTING_REGION, DEFAULT_REGION.key)
        return RegionSetting.entries.find { it.key == regionKey } ?: DEFAULT_REGION
    }

    override fun setRegion(value: RegionSetting) {
        preferenceDataSource.setString(PreferenceDefs.PREF_KEY_SETTING_REGION, value.key)
    }

    override fun getCurrentTheme(): ThemeSetting {
        val themeKey = preferenceDataSource.getString(PreferenceDefs.PREF_KEY_SETTING_THEME, DEFAULT_THEME.key)
